...
test('handle valid input updates exercise and returns assessment', function () {
    // GIVEN
    ...
    // THEN
    expect($assessments[0]->isCorrect())->toBeTrue();
...
test('handle invalid input do not update exercise', function () {
    // GIVEN
    $user = $this->createUser();
    $exercise = Exercise::factory()->create(['status' => ExerciseStatus::NEW, 'user_id' => $user->getId()]);
    $u_exercise = UnscrambleWordExercise::factory()->create(['exercise_id' => $exercise->id]);
    ...
    // THEN
    expect($assessments[0]->isCorrect())->toBeFalse();
    $this->assertDatabaseHas('learning_session_flashcards', [
        'id' => $session_flashcard->id,
        'rating' => null,
    ]);
});
test('handle learning session finished do not update rating', function () {
    // GIVEN
    $user = $this->createUser();
